...
module.exports = {
	...
	async execute(message, args, client) {
		...
		if (!isNaN(ms) && args[2]) {
			...
			await user.send({ content: `**You've been banned from ${message.guild.name} for ${args[1]}. Reason: ${args.slice(2).join(' ')}**` }).catch(e => {
			...
			await member.ban({ reason: `Banned user: ${user.tag} from ${message.guild.name} for ${args[1]}. Reason: ${args.slice(2).join(' ')}` }).catch(e => message.channel.send({ content: `\`${`${e}`.split('at')[0]}\`` }));
		}
		else if (!isNaN(ms)) {
			Embed.setTitle(`Banned ${user.tag} for ${args[1]}.`);
			await user.send({ content: `**You've been banned on ${message.guild.name} for ${args[1]}.**` }).catch(e => {
				message.channel.send({ content: 'Could not DM user! You may have to manually let them know that they have been banned.' });
			});
			client.logger.info(`Banned user: ${user.tag} on ${message.guild.name} for ${args[1]}`);
			client.memberdata.set(`${user.id}-${message.guild.id}`, Date.now() + ms, 'bannedUntil');
			await member.ban({ reason: `Banned user: ${user.tag} on ${message.guild.name} for ${args[1]}` }).catch(e => message.channel.send({ content: `\`${`${e}`.split('at')[0]}\`` }));
		}
		else if (args[1]) {
			Embed.setTitle(`Banned ${user.tag} for ${args.slice(1).join(' ')}`);
			await user.send({ content: `**You've been banned on ${message.guild.name} for ${args.slice(1).join(' ')}**` }).catch(e => {
				message.channel.send({ content: 'Could not DM user! You may have to manually let them know that they have been banned.' });
			});
			client.logger.info(`Banned user: ${user.tag} on ${message.guild.name} for ${args.slice(1).join(' ')} forever`);
			await member.ban({ reason: `Banned user: ${user.tag} on ${message.guild.name} for ${args.slice(1).join(' ')} forever` }).catch(e => message.channel.send({ content: `\`${`${e}`.split('at')[0]}\`` }));
		}
		else {
			Embed.setTitle(`Banned ${user.tag} forever.`);
			await user.send({ content: `**You've been banned on ${message.guild.name} forever.**` }).catch(e => {
				message.channel.send({ content: 'Could not DM user! You may have to manually let them know that they have been banned.' });
			});
			client.logger.info(`Banned user: ${user.tag} on ${message.guild.name} forever`);
			await member.ban({ reason: `Banned user: ${user.tag} on ${message.guild.name} forever` }).catch(e => message.channel.send({ content: `\`${`${e}`.split('at')[0]}\`` }));
		}
