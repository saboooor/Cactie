...
export default async (client: Client, role: Role) => {
  ...
  if (!srvconfig.auditlogs.logs.roledelete && !srvconfig.auditlogs.logs.role && !srvconfig.auditlogs.logs.all) return;
  let logchannelId;
  if (srvconfig.auditlogs.logs.roledelete && srvconfig.auditlogs.logs.roledelete.channel != 'false') logchannelId = srvconfig.auditlogs.logs.roledelete.channel;
