...
export default async function createRPS(user: User, opponent: User, interaction: ButtonInteraction | CommandInteraction) {
  ...
  const rpsmsg = await interaction.editReply({ components: [RPSContainer], flags: MessageFlags.IsComponentsV2 });
  ...
  const againButton = new ButtonBuilder()
    .setCustomId(`rockpaperscissors_again|${user.id}|${opponent.id}`)
