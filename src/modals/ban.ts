...
export async function execute(interaction: ModalSubmitInteraction, client: Client, memberId: Snowflake) {
	try {
		// Check if guild
		...
		if (!member)
			return error("Invalid member! Are they in this server?", interaction, true);
		...
		if (member.roles.highest.rawPosition > author.roles.highest.rawPosition)
			return error(`You can't do that! Your role is ${member.roles.highest.rawPosition - author.roles.highest.rawPosition} positions lower than the user's role!`, interaction, true);
		if (member.roles.highest.rawPosition > interaction.guild.members.me!.roles.highest.rawPosition)
			return error(`I can't do that! My role is ${member.roles.highest.rawPosition - interaction.guild.members.me!.roles.highest.rawPosition} positions lower than the user's role!`, interaction, true);
		...
		if (time > 31536000000)
			return error('You cannot ban someone for more than 1 year!', interaction, true);
