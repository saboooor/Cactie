...
export default async function resolveTicket(member: GuildMember, channel: GuildTextBasedChannel) {
  ...
  if (ticketUserIds.includes(member.id)) throw new Error('You cannot resolve this ticket! Try closing the ticket instead');

  // Check if ticket is closed
  if (channel.name.startsWith('closed')) throw new Error('This ticket is already closed!');

  // Set resolved to true
  await prisma.tickets.update({ where: { channelId: channel.id }, data: { resolved: 'true' } });

  // Send message to ticket and log
  await channel.send({ content: `${ticketUserIds.map((u: string) => { return `<@${u}>`; }).join(', ')}, this ticket has been resolved and will auto-close at 8PM Eastern Time if you do not respond.\nIf you still have an issue, please explain it here. Otherwise, you may close this ticket now.` });
