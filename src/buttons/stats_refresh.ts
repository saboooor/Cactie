...
export const name = 'stats_refresh';
...
export async function execute(interaction: ButtonInteraction) {
	try {
		...
		if (!StatsEmbed.toJSON().title && pong.hostname)
			StatsEmbed.setTitle(pong.hostname);
		else if (!StatsEmbed.toJSON().title && pong.port == 25565)
			StatsEmbed.setTitle(pong.ip);
		else if (!StatsEmbed.toJSON().title)
			StatsEmbed.setTitle(`${pong.ip}:${pong.port}`);
		if (pong.debug.cachetime)
			StatsEmbed.setDescription(`Last Pinged: <t:${pong.debug.cachetime}:R>`);
		else
			StatsEmbed.setDescription(`Last Pinged: <t:${Math.round(Date.now() / 1000)}:R>`);
		if (pong.version)
			StatsEmbed.addFields([{ name: '**Version:**', value: pong.version, inline: true }]);
		if (pong.protocol != -1 && pong.protocol)
			StatsEmbed.addFields([{ name: '**Protocol:**', value: `${pong.protocol} (${protocols[pong.protocol as keyof typeof protocols]})`, inline: true }]);
		if (pong.software)
			StatsEmbed.addFields([{ name: '**Software:**', value: pong.software, inline: true }]);
		if (pong.players)
			StatsEmbed.addFields([{ name: '**Players Online:**', value: `${pong.players.online} / ${pong.players.max}`, inline: true }]);
		...
		if (pong.motd)
			StatsEmbed.addFields([{ name: '**MOTD:**', value: pong.motd.clean.join('\n').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&le;/g, '≤').replace(/&ge;/g, '≥') }]);
		...
		if (!pong.debug.query)
			StatsEmbed.setFooter({ text: 'Query disabled! If you want more info, contact the owner to enable query.' });
