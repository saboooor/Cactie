...
export async function execute(interaction: ButtonInteraction) {
	...
	collector.on('collect', async (btninteraction) => {
		...
		if (win.rows)
			win.rows.forEach(i => btns[i].setStyle(ButtonStyle.Success));
		if (win.winner) {
	...
	collector.on('end', () => {
		if (TicTacToe.toJSON().fields![0].name == 'Result:')
			return;
