...
export const tictactoe_again: Button<'cached'> = {
  execute: async (interaction, _, args) => {
    ...
    if (!xuserId || !ouserId) {
      interaction.reply({ content: 'An error occurred while trying to start a new game. Please do /tictactoe again', flags: MessageFlags.Ephemeral });
      return;
    }
    if (xuserId != interaction.user.id && ouserId != interaction.user.id) {
      interaction.reply({ content: 'You\'re not in this game!\nCreate a new one with the /tictactoe command', flags: MessageFlags.Ephemeral });
      return;
    }
