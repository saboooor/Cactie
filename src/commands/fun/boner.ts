...
export const boner: SlashCommand = {
  ...
  async execute(message, args) {
    try {
      ...
      if (srvs[message.channel!.id] && srvs[message.channel!.id].msg) srvs[message.channel!.id].msg.delete().catch((err: Error) => logger.warn(err));
      ...
      srvs[message.channel!.id].fields.push({
        name: `${args[0] ? args.join(' ') : (message.member as GuildMember).displayName}'s pp size`,
        value: '8D',
        number: Math.round(Math.random() * Number(srvconfig.maxppsize)),
      });
      ...
      for (let step = 0; step < srvs[message.channel!.id].fields[i].number; step++) {
        ...
        if (srvs[message.channel!.id].largestNumber == srvs[message.channel!.id].fields[i].number && srvs[message.channel!.id].msg) {
          srvs[message.channel!.id].msg.edit({ embeds: [ppEmbed] }).catch((err: Error) => logger.warn(err));
        }
      }
      await sleep(1000);
      ...
      if (Math.round(Math.random() * 10) == 5) srvs[message.channel!.id].fields[i].value = `SIKE\n${args[0] ? args.join(' ') : (message.member as GuildMember).displayName} has ${hard == 1 ? 'no pp' : 'erectile dysfunction'}`;
      ...
      ppEmbed.setFields(srvs[message.channel!.id].fields);
      if (srvs[message.channel!.id].msg) srvs[message.channel!.id].msg.edit({ embeds: [ppEmbed] }).catch((err: Error) => logger.warn(err));
