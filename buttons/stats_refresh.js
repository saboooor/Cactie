		...
		try {
			...
			if (server && server.id) {
				...
				if (server.client) {
					...
					if (duration) StatsEmbed.addField({ name: '**Last Started:**', value: duration, inline: true });
				}
				if (info.node) StatsEmbed.addField({ name: '**Node:**', value: info.node, inline: true });
				if (info.docker_image) StatsEmbed.addField({ name: '**Docker Image:**', value: info.docker_image, inline: true });
				if (usages.resources.cpu_absolute) StatsEmbed.addField({ name: '**CPU Usage:**', value: `${usages.resources.cpu_absolute}% / ${info.limits.cpu}%`, inline: true });
				if (usages.resources.memory_bytes) StatsEmbed.addField({ name: '**RAM Usage:**', value: `${Math.round(usages.resources.memory_bytes / 1048576)} MB / ${info.limits.memory} MB`, inline: true });
				if (usages.resources.network_tx_bytes) StatsEmbed.addField({ name: '**Network Sent:**', value: `${Math.round(usages.resources.network_tx_bytes / 1048576)} MB`, inline: true });
				if (usages.resources.network_rx_bytes) StatsEmbed.addField({ name: '**Network Recieved:**', value: `${Math.round(usages.resources.network_rx_bytes / 1048576)} MB`, inline: true });
			...
			if (server.ip) {
				...
				if (pong.version) StatsEmbed.addField({ name: '**Version:**', value: pong.version, inline: true });
				if (pong.protocol != -1 && pong.protocol) StatsEmbed.addField({ name: '**Protocol:**', value: `${pong.protocol} (${protocols[pong.protocol]})`, inline: true });
				if (pong.software) StatsEmbed.addField({ name: '**Software:**', value: pong.software, inline: true });
				if (pong.players) StatsEmbed.addField({ name: '**Players Online:**', value: `${pong.players.online} / ${pong.players.max}`, inline: true });
				if (pong.players && pong.players.list && pong.players.online > 50) {
					...
					StatsEmbed.addField({ name: '**Players:**', value: `[Click Here](${link})`, inline: true });
				}
				else if (pong.players && pong.players.list) {
					StatsEmbed.addField({ name: '**Players:**', value: pong.players.list.join('\n').replace(/_/g, '\\_') });
				}
				if (pong.motd) StatsEmbed.addField({ name: '**MOTD:**', value: pong.motd.clean.join('\n').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&le;/g, '≤').replace(/&ge;/g, '≥') });
				if (pong.plugins && pong.plugins.raw[0]) {
					const link = await createPaste(pong.plugins.raw.join('\n'), { server: 'https://bin.birdflop.com' });
					StatsEmbed.addField({ name: '**Plugins:**', value: `[Click Here](${link})`, inline: true });
				}
