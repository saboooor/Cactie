...
module.exports = {
	...
	async execute(interaction, client) {
		try {
			...
			collector.on('end', () => {
				if (collector.collected.size == 0) interaction.message.edit({ content: `A game of ${embedJSON.title} should not last longer than an hour are you high`, components: [], embeds: [] });
			});
		}
