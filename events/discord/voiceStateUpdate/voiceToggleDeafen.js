...
const { deafen, undeafen } = require('../../../lang/int/emoji.json');
module.exports = async (client, oldState, newState) => {
	// Get current settings for the guild
	const srvconfig = await client.getData('settings', 'guildId', newState.guild.id);

	// Check if the user actually undeafened
	if (!oldState.selfDeaf && newState.selfDeaf) {
		// Check if log is enabled and send log
		if (['voicedeafen', 'voice', 'other', 'all'].some(logtype => srvconfig.auditlogs.split(',').includes(logtype))) {
			...
			const logEmbed = new EmbedBuilder()
				...
				.setTitle(`<:deafen:${deafen}> Member deafened`)
				.setFields([
